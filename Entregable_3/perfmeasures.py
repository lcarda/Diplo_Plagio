...
def count_chars2(annotations, xref, xoff, xlen):
    ...
    num_chars = 0
    max_length = max((ann[xoff] + ann[xlen] for ann in annotations), default=0)
...
def extract_annotations_from_files(path, tagname):
    ...
    for xmlfile in xmlfiles:
        #print('PERDON ESTO SE LO PEDI YO',extract_annotations_from_file(xmlfile, tagname))
...
#    print(doc.toprettyxml())
    ...
    if not doc.documentElement.hasAttribute('reference'):
       # print('Entramos al if de extract from file')
        return annotations
    
    t_ref = doc.documentElement.getAttribute('reference')
    for node in doc.documentElement.childNodes:
     ##   print('Entro al for')
#print('primer elemento el if ')
     #   print('xml.dom.Node.ELEMENT_NODE',xml.dom.Node.ELEMENT_NODE)
     #   print('node.nodeType',node.nodeType)
        
      #  print('la igualdad')
      #  print('node.hasAttribute',node.nodeType == xml.dom.Node.ELEMENT_NODE )
        #print('terna if ',(node.nodeType == xml.dom.Node.ELEMENT_NODE,node.hasAttribute('name'),node.getAttribute('name').endswith(tagname)))
        #if node.nodeType == xml.dom.Node.ELEMENT_NODE:
         #   print('la terna:',(node.nodeType == xml.dom.Node.ELEMENT_NODE,node.hasAttribute('name'),node.getAttribute('name').endswith(tagname)))
          #  print(tagname)
        if node.nodeType == xml.dom.Node.ELEMENT_NODE and \
           ...
           # print('entro al 3 if')
            ...
            if ann:
                annotations.add(ann)
            #    print('entro al if ann', ann)
    #print('esta por impirmir el return')
    return annotations
